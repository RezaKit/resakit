  ...
  return (
    <>
      ...
      <main className="bg-white pb-20 md:pb-0" style={{ fontFamily: '"Inter", -apple-system, sans-serif' }}>
        ...
        {/* ── HERO + SEARCH ─────────────────────────────────── */}
        <section className="md:bg-[#F5F2EC] md:border-b md:border-[#EFEDE8]">
          <div className="md:container md:max-w-5xl md:mx-auto md:grid md:grid-cols-[1fr_400px] md:gap-14 md:py-20 md:items-center">

            {/* Hero text */}
            <div className="px-5 pt-4 pb-2 md:px-0 md:pt-0 md:pb-0">
              ...
              <h1 className="font-serif text-[44px] md:text-[60px] leading-[0.98] tracking-[-1.2px] text-[#141414]"
              ...
              <p className="mt-3.5 text-[15px] leading-[1.45] text-[#5B5952] max-w-[300px] md:max-w-md md:text-base">
              ...
              <div className="hidden md:flex items-center gap-3 mt-8">
                <Link href="/toulouse"
                  ...
                  Explorer Toulouse <ArrowRight className="w-4 h-4" />
                </Link>
                <Link href="/pour-les-pros"
                  className="inline-flex items-center gap-2 border border-[#EFEDE8] bg-white text-[#141414] text-sm font-semibold px-6 py-3.5 rounded-xl hover:bg-[#F5F2EC] transition-colors">
                  Pour les pros
                </Link>
              </div>
            </div>
            ...
            {/* Search bar */}
            <div className="px-5 pt-4 pb-5 md:px-0 md:pt-0 md:pb-0">
              <div className="rounded-[22px] border border-[#E6E4E0] bg-white"
                style={{ boxShadow: '0 8px 24px rgba(20,20,20,0.07), 0 1px 2px rgba(0,0,0,0.03)', padding: 6 }}>
                <div className="grid grid-cols-2 px-1 pt-2.5 pb-2" style={{ borderBottom: '1px solid #F2F0EC' }}>
        ...
        <section className="pt-6 pb-2 md:py-12 bg-white">
          <div className="md:container md:max-w-5xl md:mx-auto">
            <div className="flex items-baseline justify-between px-5 mb-3 md:px-0 md:mb-6">
              <h2 className="font-serif text-[22px] md:text-3xl tracking-[-0.5px] text-[#141414]">
            ...
            <div className="flex gap-2.5 overflow-x-auto px-5 pb-1 md:hidden" style={{ scrollbarWidth: 'none' }}>
              {OCCASIONS.map((occ) => (
                <OccasionChip key={occ.label} occ={occ} />
              ))}
            </div>
            {/* Desktop: 4-col grid */}
            <div className="hidden md:grid md:grid-cols-4 md:gap-4">
              {OCCASIONS.map((occ) => (
                <OccasionChip key={occ.label} occ={occ} desktop />
              ))}
            </div>
          </div>
        </section>
        ...
        {/* ── EXPÉRIENCES ───────────────────────────────────── */}
        <section className="pt-5 pb-2 md:py-12 md:bg-[#F5F2EC]">
          <div className="md:container md:max-w-5xl md:mx-auto">
            <div className="flex items-baseline justify-between px-5 mb-3.5 md:px-0 md:mb-6">
              <h2 className="font-serif text-[22px] md:text-3xl tracking-[-0.5px] text-[#141414]">
            ...
            <div className="hidden md:grid md:grid-cols-3 md:gap-4">
              {experiences && experiences.length > 0
                ? experiences.slice(0, 3).map((exp: any) => (
                    <ExperienceCard key={exp.id} experience={exp} variant="list" />
                  ))
                : SAMPLE_EXPERIENCES.slice(0, 3).map((e, i) => (
                    <SampleListCard key={i} e={e} />
                  ))}
            </div>

            <div className="px-5 mt-4 md:px-0 md:mt-6">
              <Link href="/toulouse"
                className="flex items-center justify-center gap-2 w-full md:w-auto md:inline-flex py-3 md:px-6 md:py-3.5 rounded-2xl text-[14px] font-semibold text-[#7C3AED] border border-[#EFEDE8] bg-white hover:bg-[#F5F2EC] transition-colors">
        ...
        {/* ── SPLIT PAYMENT ─────────────────────────────────── */}
        <section className="px-5 pt-8 pb-2 md:py-16">
          <div className="md:container md:max-w-5xl md:mx-auto">
            <div className="relative rounded-[26px] overflow-hidden text-white px-5 py-6 md:px-10 md:py-10"
              ...
              <div className="relative md:grid md:grid-cols-[1fr_320px] md:gap-12 md:items-center">
                {/* Left: text */}
                <div>
                  <div className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full mb-4 text-[10px] font-semibold tracking-[0.4px] uppercase"
                  ...
                  <h3 className="font-serif text-[32px] md:text-[42px] leading-none tracking-[-0.8px] mb-0">
                    Une réservation.<br />
                    <em className="italic" style={{ color: '#FFB27A' }}>Une addition partagée.</em>
                  </h3>
                  <p className="mt-3 text-[13.5px] md:text-[15px] leading-[1.5] md:max-w-md" style={{ color: 'rgba(255,255,255,0.78)' }}>
                  ...
                  <div className="hidden md:flex items-center gap-4 mt-8">
                    <Link href="/toulouse"
                      className="inline-flex items-center gap-2 bg-white text-[#141414] text-sm font-semibold px-5 py-3 rounded-xl hover:bg-[#F5F2EC] transition-colors">
                      Voir les expériences <ArrowRight className="w-4 h-4" />
                    </Link>
                  </div>
                </div>

                {/* Right: participants live */}
                <div className="mt-4 md:mt-0 rounded-2xl p-3.5"
        ...
        {/* ── HOW IT WORKS ──────────────────────────────────── */}
        <section className="px-5 pt-10 pb-4 md:py-20">
        ...
        {/* ── WAITLIST ──────────────────────────────────────── */}
        <section className="px-5 pt-8 pb-10 md:py-16">
          <div className="md:container md:max-w-2xl md:mx-auto">
            <div className="rounded-3xl border border-orange-100 p-8 md:p-12 text-center" style={{ background: '#FFF7EF' }}>
...
function OccasionChip({ occ, desktop }: { occ: typeof OCCASIONS[0]; desktop?: boolean }) {
  return (
    <Link href={`/toulouse?occasion=${occ.label}`}
      className="flex-shrink-0 flex flex-col gap-0.5 rounded-2xl relative hover:opacity-90 transition-opacity"
      style={{
        padding: desktop ? '16px 40px 16px 20px' : '13px 36px 12px 16px',
        background: occ.bg,
        border: '1px solid rgba(0,0,0,0.03)',
        minWidth: desktop ? undefined : 150,
      }}>
...
/* Desktop grid card — full width, 180px image (matches ExperienceCard list variant) */
function SampleListCard({ e }: { e: typeof SAMPLE_EXPERIENCES[0] }) {
  return (
    <div className="rounded-2xl overflow-hidden bg-white border border-[#EFEDE8]">
      <div className="relative h-[180px] overflow-hidden" style={{ background: `url(${e.img}) center/cover` }}>
        <div className="absolute inset-0" style={{ background: 'linear-gradient(180deg, transparent 60%, rgba(0,0,0,0.35))' }} />
        <div className="absolute top-2.5 left-2.5 flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-semibold text-[#141414]"
          style={{ background: 'rgba(255,255,255,0.95)' }}>
          <span className="text-[#7C3AED]">◆</span> {e.tag}
        </div>
