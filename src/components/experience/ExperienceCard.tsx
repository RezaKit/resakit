...
export function ExperienceCard({ experience, city = 'toulouse', className }: ExperienceCardProps) {
  ...
  return (
    <Link
      href={href}
      className={`group block rounded-xl overflow-hidden bg-white border border-gray-200 hover:shadow-lg hover:-translate-y-1 transition-all duration-200 ${className || ''}`}
    >
      <div className="relative aspect-[4/3] overflow-hidden bg-gray-100">
        ...
        <div className="absolute top-3 left-3">
          <Badge variant="default">{getCategoryLabel(experience.tags?.[0] || '')}</Badge>
        </div>
        {experience.occasions?.includes('EVJF' as any) && (
          <div className="absolute top-3 right-3">
            <Badge variant="secondary">EVJF</Badge>
          </div>
        )}
      </div>
      ...
      <div className="p-4">
        ...
        <div className="flex items-center gap-4 text-sm text-gray-600 mb-3">
          ...
          <span className="flex items-center gap-1">
            ...
            {experience.min_people}-{experience.max_people}
          </span>
        </div>
        ...
        <div className="flex items-center justify-between pt-3 border-t border-gray-100">
          {experience.rating_count > 0 ? (
            <span className="flex items-center gap-1 text-sm">
              ...
              <span className="font-medium">{experience.rating_average}</span>
              <span className="text-gray-500">({experience.rating_count})</span>
            </span>
          ) : (
            <span className="text-sm text-gray-400">Nouveau</span>
          )}
          <span className="font-bold text-brand-violet">{priceDisplay}</span>
        </div>
      </div>
    </Link>
  )
